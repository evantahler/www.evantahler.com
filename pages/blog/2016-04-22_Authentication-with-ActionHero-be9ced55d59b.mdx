...
_// Note! The real name of your migration must be in the sequelize's timestamp format, and look something more like \`20130326205332-addUserTable.json\`_
_// It would be best to use the \`sequelize\` binary to build your migration file._
...
#### **initilizers/mysql.js**

**var** fs **\=** require('fs');
exports.mysql **\=** **function**(api, next){

api.SequelizeBase **\=** require("sequelize");
api.sequelize **\=** **new** api.SequelizeBase( api.configData.mySQL.database, api.configData.mySQL.username, api.configData.mySQL.password, {
...
api.models\[model\] **\=** api.sequelize.**import**(\_\_dirname **+** "./../models/"**+**model**+**".js");
});

**var** initDB **\=** **function**(next){
**var** migrator **\=** api.sequelize.getMigrator({path**:** process.cwd() **+** "/migrations"}, **true**)
...
_// check to ensure the user is still ok in the DB_
...
**var** passwordHash **\=** crypto.createHash('sha256').update(user.passwordSalt **+** connection.params.password).digest("hex");
...
connection.response\[api.configData.commonWeb.fingerprintOptions.cookieKey\] **\=** connection.id;
...
**var** newData **\=** {};
**if**(connection.params.email **!=** **null**){ newData.email **\=** connection.params.email; }
**if**(connection.params.firstName **!=** **null**){ newData.firstName **\=** connection.params.firstName; }
**if**(connection.params.lastName **!=** **null**){ newData.lastName **\=** connection.params.lastName; }
...
_Originally published at 18 Feb 2013_
