...
_// you can pass a FQDN here, or function to be called / window object to be inspected_
clientUrl**:** 'window.location.origin',
_// Directory to render client-side JS._
_// Path should start with "/" and will be built starting from api.config..general.paths.public_
clientJsPath**:** 'javascript/',

_// the name of the client-side JS file to render. Both \`.js\` and \`.min.js\` versions will be created_
_// do not include the file exension_
_// set to \`null\` to not render the client-side JS on boot_
...
_// Which channel to use on redis pub/sub for RPC communication_
channel**:** 'actionhero',
_// How long to wait for an RPC call before considering it a failure_
rpcTimeout**:** 5000,
